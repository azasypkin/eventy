...
define(["config"], function (config) {
	...
	return {
		...
		// The format string uses the same abbreviations as in this.getDateFromFormat()
		// ------------------------------------------------------------------
		formatDate: function (date,format) {
			...
			while (i_format < format.length) {
				...
				if (value[token] !== null && typeof value[token] !== "undefined") { result=result + value[token]; }
		...
		// ------------------------------------------------------------------
		// Utility functions for parsing in this.getDateFromFormat()
		// ------------------------------------------------------------------
		...
		// ------------------------------------------------------------------
		// this.getDateFromFormat( date_string , format_string )
		//
		...
		getDateFromFormat: function (val,format) {
			...
			while (i_format < format.length) {
				...
				if (token==="yyyy" || token==="yy" || token==="y") {
					...
					if (year===null || typeof year === "undefined") { return 0; }
					i_val += year.length;
					if (year.length===2) {
		...
		getUtcDateFromFormat: function (val, format, offset) {
			return this.utcDate(this.getDateFromFormat(val, format), offset);
		},

		// date is the Date object and offset should be expressed in minutes
		//[BUG]: We don't consider DST
		offsetDate: function (date, offset) {
			// Express this local time as the number of milliseconds since Jan. 1, 1970,
			//then obtain local UTC offset and convert to msec (1000 milliseconds = 1 second, and 1 minute = 60 seconds)
			// Therefore, converting minutes to milliseconds involves multiplying by 60 * 1000 = 60000. 
			// obtain UTC time in msec
			var utcDate;
			if (typeof date === 'object') {
				utcDate = date.getTime() + (date.getTimezoneOffset() * 60000);
			} else {
				utcDate = date;
			}
			return utcDate + (60000 * offset);
		},
